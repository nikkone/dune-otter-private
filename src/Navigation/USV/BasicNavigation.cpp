...
namespace Navigation
{
  namespace USV
  {
    ...
    BasicNavigation::BasicNavigation(const std::string& name, DUNE::Tasks::Context& ctx):
      ...
      m_avg_gps(NULL)
    {
      ...
      .visibility(DUNE::Tasks::Parameter::VISIBILITY_USER)
      .units(DUNE::Units::SquareMeter)
      .defaultValue("500.0")
      .minimumValue("100.0")
      ...
      m_gvel_val_bits = DUNE::IMC::GroundVelocity::VAL_VEL_X
                        | DUNE::IMC::GroundVelocity::VAL_VEL_Y
                        | DUNE::IMC::GroundVelocity::VAL_VEL_Z;

      m_wvel_val_bits = DUNE::IMC::WaterVelocity::VAL_VEL_X
                        | DUNE::IMC::WaterVelocity::VAL_VEL_Y
                        | DUNE::IMC::WaterVelocity::VAL_VEL_Z;

      // Register callbacks.
      bind<DUNE::IMC::Acceleration>(this);
      bind<DUNE::IMC::AngularVelocity>(this);
      bind<DUNE::IMC::DataSanity>(this);
      bind<DUNE::IMC::Depth>(this);
      bind<DUNE::IMC::DepthOffset>(this);
      bind<DUNE::IMC::Distance>(this);
      bind<DUNE::IMC::EulerAngles>(this);
      bind<DUNE::IMC::EulerAnglesDelta>(this);
      bind<DUNE::IMC::GpsFix>(this);
      bind<DUNE::IMC::GroundVelocity>(this);
      bind<DUNE::IMC::LblConfig>(this);
      bind<DUNE::IMC::LblRange>(this);
      bind<DUNE::IMC::Rpm>(this);
      bind<DUNE::IMC::UsblFixExtended>(this);
      bind<DUNE::IMC::WaterVelocity>(this);
    }
    ...
    {
      m_avg_heave = new DUNE::Math::MovingAverage<double>(m_avg_heave_samples);
      m_avg_gps = new DUNE::Math::MovingAverage<double>(m_avg_gps_samples);
      reset();
    }
    ...
    {
      DUNE::Memory::clear(m_origin);
      DUNE::Memory::clear(m_avg_heave);
      DUNE::Memory::clear(m_avg_gps);
    }

    void
    BasicNavigation::consume(const DUNE::IMC::Acceleration* msg)
    {
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::AngularVelocity* msg)
    {
      if (msg->getSourceEntity() != m_ahrs_eid)
        return;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::Depth* msg)
    {
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::DepthOffset* msg)
    {
      if (msg->getSourceEntity() != m_depth_eid)
        return;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::DataSanity* msg)
    {
      if (msg->getSourceEntity() != m_dvl_eid)
        return;

      if (msg->sane == DUNE::IMC::DataSanity::DS_NOT_SANE)
      {
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::Distance* msg)
    {
      if (msg->getSourceEntity() != m_alt_eid)
        return;

      if (msg->validity == DUNE::IMC::Distance::DV_INVALID)
        return;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::EulerAngles* msg)
    {
      if (msg->getSourceEntity() != m_ahrs_eid)
        return;

      if (std::fabs(msg->phi) > DUNE::Math::c_pi ||
          std::fabs(msg->theta) > DUNE::Math::c_pi ||
          std::fabs(msg->psi) > DUNE::Math::c_pi)
      {
      ...
      m_euler_bfr[AXIS_Z] += getEuler(AXIS_Z) + DUNE::Math::Angles::minSignedAngle(getEuler(AXIS_Z), msg->psi);
      ++m_euler_readings;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::EulerAnglesDelta* msg)
    {
      if (msg->getSourceEntity() != m_imu_eid)
        return;

      if (std::fabs(msg->x) > DUNE::Math::c_pi / 10.0 ||
          std::fabs(msg->y) > DUNE::Math::c_pi / 10.0 ||
          std::fabs(msg->z) > DUNE::Math::c_pi / 10.0)
      {
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::GpsFix* msg)
    {
      ...
      // Check fix validity.
      if ((msg->validity & DUNE::IMC::GpsFix::GFV_VALID_POS) == 0)
      {
        m_gps_rej.reason = DUNE::IMC::GpsFixRejection::RR_INVALID;
      ...
      {
        m_gps_rej.reason = DUNE::IMC::GpsFixRejection::RR_ABOVE_THRESHOLD;
        dispatch(m_gps_rej, DF_KEEP_TIME);
        return;
      }

      // Check if we have valid Horizontal Accuracy index.
      if (msg->validity & DUNE::IMC::GpsFix::GFV_VALID_HACC)
      {
        ...
        {
          m_gps_rej.reason = DUNE::IMC::GpsFixRejection::RR_ABOVE_MAX_HACC;
          dispatch(m_gps_rej, DF_KEEP_TIME);
          return;
        }
      }
      else
      {
        ...
        {
          m_gps_rej.reason = DUNE::IMC::GpsFixRejection::RR_ABOVE_MAX_HDOP;
          dispatch(m_gps_rej, DF_KEEP_TIME);
          return;
        }
      }
      ...
      // Speed over ground.
      if (msg->validity & DUNE::IMC::GpsFix::GFV_VALID_SOG)
        m_gps_sog = msg->sog;
      ...
      double x = 0.0;
      double y = 0.0;
      DUNE::Coordinates::WGS84::displacement(m_origin->lat, m_origin->lon, m_origin->height,
                                       msg->lat, msg->lon, msg->height,
                                       &x, &y, &m_last_z);

      // Stream Estimator.
      DUNE::IMC::EstimatedStreamVelocity stream;
      ...
      if (DUNE::Math::norm(x, y) > m_max_dis2ref)
      {
        // Redefine origin.
        DUNE::Memory::replace(m_origin, new DUNE::IMC::GpsFix(*msg));

        // Recalculate LBL positions.
        m_ranging.updateOrigin(msg);

        // Save message to cache.
        DUNE::IMC::CacheControl cop;
        cop.op = DUNE::IMC::CacheControl::COP_STORE;
        cop.message.set(*msg);
        dispatch(cop);
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::GroundVelocity* msg)
    {
      m_gvel = *msg;
      ...
      m_dvl_rej.type = DUNE::IMC::DvlRejection::TYPE_GV;
      ...
      if ((tstep > 0) && (tstep < m_dvl_time_rel_thresh))
      {
        ...
        {
          m_dvl_rej.reason = DUNE::IMC::DvlRejection::RR_INNOV_THRESHOLD_X;
        ...
        {
          m_dvl_rej.reason = DUNE::IMC::DvlRejection::RR_INNOV_THRESHOLD_Y;
      ...
      {
        m_dvl_rej.reason = DUNE::IMC::DvlRejection::RR_ABS_THRESHOLD_X;
      ...
      {
        m_dvl_rej.reason = DUNE::IMC::DvlRejection::RR_ABS_THRESHOLD_Y;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::LblConfig* msg)
    {
      if (msg->op != DUNE::IMC::LblConfig::OP_SET_CFG)
          return;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::LblRange* msg)
    {
      if (!m_active)
        return;
      ...
      {
        m_lbl_ac.acceptance = DUNE::IMC::LblRangeAcceptance::RR_NO_INFO;
      ...
      {
        m_lbl_ac.acceptance = DUNE::IMC::LblRangeAcceptance::RR_AT_SURFACE;
        dispatch(m_lbl_ac, DF_KEEP_TIME);
        return;
      }
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::Rpm* msg)
    {
      m_rpm = msg->value;
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::UsblFixExtended* msg)
    {
      ...
      double x = 0.0;
      double y = 0.0;
      DUNE::Coordinates::WGS84::displacement(m_origin->lat, m_origin->lon, 0.0,
                                       msg->lat, msg->lon, 0.0,
                                       &x, &y);
    ...
    void
    BasicNavigation::consume(const DUNE::IMC::WaterVelocity* msg)
    {
      m_wvel = *msg;
      // Correct for the distance between center of gravity and dvl.
      ...
      m_dvl_rej.setTimeStamp(msg->getTimeStamp());
      m_dvl_rej.type = DUNE::IMC::DvlRejection::TYPE_WV;
    ...
    void
    BasicNavigation::startNavigation(const DUNE::IMC::GpsFix* msg)
    {
      DUNE::Memory::replace(m_origin, new DUNE::IMC::GpsFix(*msg));
      ...
      setEntityState(DUNE::IMC::EntityState::ESTA_BOOT, DUNE::Status::CODE_WAIT_CONVERGE);
    }
    ...
    BasicNavigation::reset(void)
    {
      ...
      setEntityState(DUNE::IMC::EntityState::ESTA_BOOT, DUNE::Status::CODE_WAIT_GPS_FIX);
    ...
    BasicNavigation::runKalmanLBL(int beacon, float range, double dx, double dy, double exp_range)
    {
      ...
      DUNE::Math::Matrix H(1, 2, 0.0);
      H(0, 0) = dx / exp_range;
      H(0, 1) = dy / exp_range;
      DUNE::Math::Matrix P(2, 2, 0.0);
      ...
      {
        m_lbl_ac.acceptance = DUNE::IMC::LblRangeAcceptance::RR_ABOVE_THRESHOLD;
        dispatch(m_lbl_ac, DF_KEEP_TIME);
        return;
      }
      else
      {
        ...
        m_lbl_ac.acceptance = DUNE::IMC::LblRangeAcceptance::RR_ACCEPTED;
        dispatch(m_lbl_ac, DF_KEEP_TIME);
        m_lbl_reading = true;
      }
    }
    ...
    BasicNavigation::onDispatchNavigation(void)
    {
      ...
      m_estate.phi = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_X));
      m_estate.theta = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_Y));
      ...
      DUNE::Coordinates::BodyFixedFrame::toInertialFrame(m_estate.phi, m_estate.theta, m_estate.psi,
    ...
    BasicNavigation::isActive(void)
    {
      ...
      if (gotEulerReadings())
      {
        ...
        estate.phi = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_X));
        estate.theta = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_Y));
        estate.psi = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_Z));
    ...
    {
      double tstamp = DUNE::Time::Clock::getSinceEpoch();
    ...
    BasicNavigation::checkUncertainty(bool abort)
    {
      ...
      if (hpos_var > m_max_hpos_var)
      {
        switch (m_navstate)
        {
          ...
          case SM_STATE_NORMAL:
            if (abort)
            {
              setEntityState(DUNE::IMC::EntityState::ESTA_ERROR, getUncertaintyMessage(hpos_var));
      ...
      else
      {
        ...
        {
          case SM_STATE_BOOT:
            setEntityState(DUNE::IMC::EntityState::ESTA_NORMAL, DUNE::Status::CODE_ACTIVE);
            break;
          case SM_STATE_NORMAL:
            if (m_depth_sensor && m_time_without_depth.overflow())
            {
              setEntityState(DUNE::IMC::EntityState::ESTA_ERROR, DUNE::Utils::String::str(DTR("no measurements available: %s"), DTR("Depth")));
              return;
            }

            if (m_time_without_euler.overflow())
            {
              setEntityState(DUNE::IMC::EntityState::ESTA_ERROR, DUNE::Utils::String::str(DTR("no measurements available: %s"), DTR("Euler Angles")));
              return;
            }
            ...
            {
              if (m_aligned)
                setEntityState(DUNE::IMC::EntityState::ESTA_NORMAL, DUNE::Status::CODE_ALIGNED);
              else
                setEntityState(DUNE::IMC::EntityState::ESTA_NORMAL, DUNE::Status::CODE_NOT_ALIGNED);
            }
            else
            {
              setEntityState(DUNE::IMC::EntityState::ESTA_NORMAL, DUNE::Status::CODE_ACTIVE);
    ...
    BasicNavigation::checkDeclination(double lat, double lon, double height)
    {
      if (!m_declination_defined && m_use_declination)
      {
        ...
        DUNE::Coordinates::WMM wmm(m_ctx.dir_cfg);
    ...
    BasicNavigation::extractEarthRotation(double& p, double& q, double& r)
    {
      ...
      DUNE::Math::Matrix ea(3,1);
      ea(0) = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_X));
      ea(1) = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_Y));
      ea(2) = DUNE::Math::Angles::normalizeRadian(getEuler(AXIS_Z));

      // Earth rotation vector.
      DUNE::Math::Matrix we(3,1);
      we(0) = DUNE::Math::c_earth_rotation * std::cos(m_last_lat);
      we(1) = 0.0;
      we(2) = - DUNE::Math::c_earth_rotation * std::sin(m_last_lat);

      // Sensed angular velocities due to Earth rotation effect.
      DUNE::Math::Matrix av(3,1);
