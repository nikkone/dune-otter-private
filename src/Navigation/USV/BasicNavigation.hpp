...
#ifndef DUNE_NAVIGATION_USV_HPP_INCLUDED_
#define DUNE_NAVIGATION_USV_HPP_INCLUDED_
...
namespace Navigation
{
  namespace USV
  {
    ...
    class BasicNavigation: public DUNE::Tasks::Periodic
    {
    public:
      //! Constructor.
      //! @param[in] name name.
      //! @param[in] ctx context.
      BasicNavigation(const std::string& name, DUNE::Tasks::Context& ctx);
      ...
      void
      consume(const DUNE::IMC::Acceleration* msg);

      void
      consume(const DUNE::IMC::AngularVelocity* msg);

      void
      consume(const DUNE::IMC::DataSanity* msg);

      void
      consume(const DUNE::IMC::Distance* msg);

      void
      consume(const DUNE::IMC::Depth* msg);

      void
      consume(const DUNE::IMC::DepthOffset* msg);

      void
      consume(const DUNE::IMC::EulerAngles* msg);

      void
      consume(const DUNE::IMC::EulerAnglesDelta* msg);

      void
      consume(const DUNE::IMC::GpsFix* msg);

      void
      consume(const DUNE::IMC::GroundVelocity* msg);

      void
      consume(const DUNE::IMC::LblConfig* msg);

      void
      consume(const DUNE::IMC::LblRange* msg);

      void
      consume(const DUNE::IMC::Rpm* msg);

      void
      consume(const DUNE::IMC::UsblFixExtended* msg);

      void
      consume(const DUNE::IMC::WaterVelocity* msg);
    ...
    protected:
      ...
      //! Stream Estimator.
      DUNE::Navigation::StreamEstimator m_stream_filter;
      //! Propeller speed (RPM)
      int16_t m_rpm;
      ...
      DUNE::IMC::LblRangeAcceptance m_lbl_ac;
      //! GPS fix rejection.
      DUNE::IMC::GpsFixRejection m_gps_rej;
      //! DVL measurement rejection.
      DUNE::IMC::DvlRejection m_dvl_rej;
      //! Navigation Uncertainty log.
      DUNE::IMC::NavigationUncertainty m_uncertainty;
      //! Navigation Data log.
      DUNE::IMC::NavigationData m_navdata;
      //! Current velocity relative to ground message.
      DUNE::IMC::GroundVelocity m_gvel;
      //! Current velocity relative to the water message.
      DUNE::IMC::WaterVelocity m_wvel;
      //! Time without GPS sensor readings deadline.
      DUNE::Time::Counter<double> m_time_without_gps;
      //! Time without DVL sensor readings deadline.
      DUNE::Time::Counter<double> m_time_without_dvl;
      //! Time without altitude readings deadline.
      DUNE::Time::Counter<double> m_time_without_alt;
      //! DVL data sanity timeout.
      DUNE::Time::Counter<double> m_dvl_sanity_timer;
      //! Time without main depth provider.
      DUNE::Time::Counter<double> m_time_without_main_depth;
      //! Time without depth readings.
      DUNE::Time::Counter<double> m_time_without_depth;
      //! Time without euler angles readings.
      DUNE::Time::Counter<double> m_time_without_euler;
      ...
      DUNE::Math::Derivative<double> m_deriv_heave;
    ...
    private:
      ...
      void
      startNavigation(const DUNE::IMC::GpsFix* msg);
      ...
      DUNE::IMC::GroundVelocity m_gvel_previous;
      //! Previous velocity relative to water message measurement.
      DUNE::IMC::WaterVelocity m_wvel_previous;
      //! Navigation Startup point.
      DUNE::IMC::GpsFix* m_origin;
      ...
      DUNE::Time::Delta m_dvl_gv_tstep;
      //! Timestep between consecutive valid water velocity messages.
      DUNE::Time::Delta m_dvl_wv_tstep;
      ...
      DUNE::Math::MovingAverage<double>* m_avg_heave;
      //! Moving Average for GpsFix.
      DUNE::Math::MovingAverage<double>* m_avg_gps;
